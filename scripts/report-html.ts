...
function render(report: Report, bank: Category[]): string {
    ...
    for (const q of cat.questions) {
      ...
      if (best) {
        ...
        const excerpt = best.answerSummary
          ...
          : "";
        parts.push(
          `<div class="excerpt">${esc(excerpt)}<em>“${esc(truncate(best.transcriptText, 320))}”</em></div>`
        );
        if (clips.length > 1) {
          const extra = clips
            .slice(1, 4)
  ...
  parts.push(
    `<p class="foot">“Confidence” is the matching model's (${esc(report.model)}) self-assessed score that the ` +
      `linked moment substantively answers the question (0–100%). Answered ≥ 60% · Partial 50–60% · ` +
      `Gap &lt; 50%. Timestamps deep-link into the source video. Generated by the SEO clip-finder spike.</p>`
  );
